...
export async function POST(request) {
  try {
    ...
    const greetingKeywords = ['hi','hlo', 'hello', 'hey', 'good morning', 'good afternoon', 'good evening', 'greetings', 'hiya'];
    ...
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are Dr. MedBot. Give ultra-concise health info in 1-2 short sentences. Use bullet points only when absolutely necessary. Be direct and clear. No emojis, no references, no citations. Always end with: "⚠️ General info only. See doctor for personal advice."',
          },
          {
            role: 'user',
            content: message,
